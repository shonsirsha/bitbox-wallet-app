...
export const BottomNavigation = ({ activeAccounts }: Props) => {
  ...
  const isInTabSection = (section: string) => {
    switch (section) {
    case 'portfolio':
  ...
  const getDestinationUrl = (tabSection: string, savedUrl: string) => {
    const isCurrentTab = isInTabSection(tabSection);
    const defaultUrl = defaultUrls[tabSection as keyof typeof defaultUrls];

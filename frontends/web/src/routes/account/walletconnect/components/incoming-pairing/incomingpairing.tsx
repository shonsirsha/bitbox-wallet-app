...
const PairingContainer = ({ pairingMetadata }: {pairingMetadata: TIncomingPairingProps['pairingMetadata']}) => {
  ...
  return (
    <div className={styles.pairingDetailContainer}>
      <div className={styles.pairingDetailTextContainer}>
        <p>{name}</p>
        <p>{description}</p>
        <p>{url}</p>
      </div>
...
export const WCIncomingPairing = ({
  ...
  const handleApprovePairing = async () => {
    ...
    Object.keys(requiredNamespaces).forEach(key => {
      requiredNamespaces[key].chains?.map(chain => accounts.push(`${chain}:${walletAddress}`));
    });
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.walletAddress}>{t('accountInfo.address')}: {walletAddress}</p>
